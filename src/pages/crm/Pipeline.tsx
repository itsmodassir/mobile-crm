...
import { ChevronLeft, Plus, Search, Loader2, Trash2, X, DollarSign, Calendar } from 'lucide-react';
...
const newDeal = () => ({ title: "", stage: "lead", value: 0, currency: localStorage.getItem('crm_currency') || "INR", expected_close: "" });
...
export function Pipeline() {
    ...
    const [form, setForm] = useState<any>(newDeal());
    ...
    const filtered = deals.filter(r => 
        ...
        (r.stage && r.stage.toLowerCase().includes(search.toLowerCase()))
    );
    ...
    const openEdit = (record: any) => {
        setForm({
            ...record,
            expected_close: record.expected_close ? new Date(record.expected_close).toISOString().split('T')[0] : ""
        });
        setIsFormOpen(true);
    };
    ...
    return (
        <PageTransition className="min-h-screen bg-slate-50/50 pb-20">
            <div className="sticky top-0 z-20 bg-background/80 backdrop-blur-xl border-b border-border/50 px-4 py-3 flex items-center gap-3 safe-top">
                ...
                <button onClick={openAdd} className="w-8 h-8 flex items-center justify-center rounded-full bg-orange-500 text-white shadow-sm active:scale-95 transition-all">
            ...
            <div className="p-4">
                <div className="relative mb-4">
                    ...
                    <input 
                        ...
                        className="w-full pl-9 pr-4 py-2.5 bg-white border border-slate-200 rounded-xl text-sm focus:outline-none focus:ring-2 focus:ring-orange-500/20 focus:border-orange-500 transition-all shadow-sm"
                    />
                </div>
                            ...
                            <div key={deal.id} className="bg-white border border-slate-200 rounded-xl p-4 shadow-sm flex flex-col gap-3">
                                <div className="flex items-start justify-between">
                                    <div className="flex-1 pr-4">
                                        <h3 className="font-semibold text-slate-900 line-clamp-1">{deal.title}</h3>
                                    ...
                                    <div className="flex items-center gap-1 shrink-0">
                                        <button onClick={() => openEdit(deal)} className="px-3 py-1 bg-slate-50 text-slate-600 rounded-lg text-xs font-medium hover:bg-slate-100 transition-colors">
                                            Edit
                                        </button>
                                        <button onClick={() => handleDelete(deal.id)} className="p-1.5 bg-rose-50 text-rose-600 rounded-lg hover:bg-rose-100 transition-colors">
                                ...
                                <div className="grid grid-cols-2 gap-2 mt-2 pt-3 border-t border-slate-100 text-xs text-slate-600">
                                    <div className="flex items-center gap-1.5 font-semibold text-orange-600">
                                        ...
                                        <span className="truncate">{deal.currency || globalCurrency} {Number(deal.value).toLocaleString()}</span>
                                    </div>
                                    <div className="flex items-center gap-1.5 truncate">
                                        <Calendar className="w-3.5 h-3.5 text-slate-400 shrink-0" />
                                        <span className="truncate">{deal.expected_close ? new Date(deal.expected_close).toLocaleDateString() : 'No date'}</span>
                                    </div>
                                </div>
                            </div>
                        ))
                ...
                <div className="fixed inset-0 z-50 flex items-end sm:items-center justify-center p-4 bg-black/50 backdrop-blur-sm">
                    <div className="bg-white w-full max-w-md rounded-2xl shadow-xl overflow-hidden animate-in slide-in-from-bottom-4 fade-in duration-200">
                        <div className="flex items-center justify-between p-4 border-b border-slate-100">
                            <h2 className="font-semibold text-lg">{form.id ? 'Edit' : 'Add'} Deal</h2>
                        ...
                        <div className="p-4 space-y-4 max-h-[70vh] overflow-y-auto">
                            <div>
                                <label className="block text-xs font-medium text-slate-500 mb-1">Deal Title *</label>
                                <input 
                                    type="text" 
                                    value={form.title} 
                        ...
                        <div className="p-4 border-t border-slate-100 bg-slate-50">
                            <button 
                                onClick={handleSave}
                                disabled={isSaving}
                                className="w-full py-3 bg-orange-600 text-white font-medium rounded-xl disabled:opacity-50 active:scale-[0.98] transition-all flex items-center justify-center"
                            >
                                {isSaving ? <Loader2 className="w-5 h-5 animate-spin" /> : 'Save Deal'}
                            </button>
                        </div>
                    </div>
                </div>
